...
/**
 * Actively check Alipay order status and complete topup if paid.
 * Called after returning from the Alipay payment page (synchronous return).
 */
export async function checkAlipayOrder(
  outTradeNo: string
): Promise<ApiResponse<string>> {
  const res = await api.get(
    `/api/alipay/check?out_trade_no=${encodeURIComponent(outTradeNo)}`,
